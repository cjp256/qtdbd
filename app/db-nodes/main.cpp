...
void parseCommandLine(QCoreApplication &app, CmdLineOptions *opts)
{
    ...
    parser.setApplicationDescription("list nodes at specified db path");
...
int main(int argc, char *argv[])
{
    ...
    QCoreApplication::setApplicationName("db-nodes");
    ...
    auto reply = dbClient->list(g_cmdLineOptions.key);
    ...
    QTextStream(stdout) << reply.value().join(" ") << endl;
    return 0;
}
